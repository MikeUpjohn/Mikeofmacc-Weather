...
/** PRINTS ADMINISTRATION VIEW OF CONFIRMATION OF ADDITION OR DELETION OF A WEATHER WARNING **/
			...
			<div class='right'>
				<div class='MTA'>";
					if($_GET['message'] == "success") {
						echo "<p>Warning added! The warning is now live on the site.</p>";
					}
					else if($_GET['message'] == "failed") {
						echo "<p>Error updating the warning. Please try again.</p>";
					}
					else if($_GET['message'] == "updateSuccessful") {
						echo "<p>Warning updated!</p>";
					}
					else if($_GET['message'] == "updateFailed") {
						echo "<p>Warning update failed.</p>";
					}
				echo "</div>
			</div>
		</div>
