...
/** DISPLAYS ADMINISTRATOR VIEW OF CONVECTIVE AND THUNDERSTORM WARNINGS **/
$query = "SELECT * FROM severeweather_convectiveforecast";
...
if(!$result) {
	...
	if($numberOfResults == 0) {
		...
		while($row = mysql_fetch_array($result)) {
			...
			echo "<td>
					<a href='edit.php?forecastType=convective&forecastID=" . $row['forecastID'] . "'>[Edit]</a>
					<a href='delete.php?forecastType=convective&forecastID=" . $row['forecastID'] . "'>[Delete]</a>
				</td>";
			echo "</tr>";
		}
		echo "</table>";
